...
from bam_masterdata.metadata.definitions import VocabularyTypeDef
from bam_masterdata.metadata.entities import BaseEntity
...
# Encrpytion key for testing
# Test-Fernet-Key erzeugen
test_key = base64.urlsafe_b64encode(Fernet.generate_key())


@pytest.fixture(autouse=True)
def set_fernet_key():
    settings.SECRET_ENCRYPTION_KEY = test_key


@pytest.fixture(autouse=True)
def set_secret_encryption_key():
    # Generiert einen gültigen 32-byte base64 Key
